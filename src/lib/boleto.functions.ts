...
const platformRecipientId =
  process.env.PLATFORM_RECIPIENT_ID;
...
if (!resolved.name) {
  throw new Error("Nome é obrigatório para boleto");
}

if (!resolved.email) {
  throw new Error("E-mail é obrigatório para boleto");
}

if (!resolved.document) {
  throw new Error("CPF ou CNPJ é obrigatório para boleto");
}

if (!validateDocument(resolved.document)) {
...
try {
  ...
  throw new Error(
    `Resposta inválida da Pagar.me: ${raw.slice(
      0,
      200
    )}`
  );
...
const {
  ...
  .insert({
    tenant_id: data.tenantId,
    cost_center_id:
      (data as any).costCenterId ?? null,
    amount:
      amounts.donationAmount / 100,
    payment_id: payment.id,
    donor_name:
      resolved.name?.trim() ?? null,
    donor_email:
      resolved.email?.trim() ?? null,
    donor_document:
      resolved.document
        ? resolved.document.replace(
            /\D/g,
            ""
          )
        : null,
